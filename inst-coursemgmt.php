  ...
  <div class="modal fade" id="createCourseModal" tabindex="-1" role="dialog" aria-labelledby="createCourseModalLabel">
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-primary btn-ok" data-dismiss="modal" id="createCourseBtn">Create Course</button>
          <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
        </div>
      </div>
    </div>
...
var getChapters = function(course){
  ...
  $.ajax({
    ...
    success: function(data){
      ...
      $("#selectedCourseOutput").html('<br><p><button id="deleteCourseBtn" class="btn btn-danger">Delete Course "'+ selectedCourse +'"</button></p>');
    }
  });
}
...
var getQuestions = function(){
  $.ajax({
    ...
    success: function(data){
      questions = [];
      $('#output').empty();
      var htmlStr = '<p><button id="deleteChapterBtn" class="btn btn-danger">Delete Chapter "'+ selectedCourse +' - ' + selectedChapter + '"</button></p><h3>'+ selectedCourse + ' - Chapter ' + selectedChapter + ' Questions</h3><table id="table" class="display table table-hover table-bordered text-left"><thead><tr><th>ID#</th><th>Question Text</th><th>Choices</th><th>Answer</th><th>Options</th></tr></thead><tbody>';
